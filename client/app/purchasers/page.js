...
import { Edit3, MessageSquareText, Plus, Search, ShoppingBag, Tags } from "lucide-react";
...
export default function PurchasersPage() {
  ...
  const groupedPurchasers = useMemo(() => {
    const groups = new Map();
    purchasers.forEach((item) => {
  ...
  const selectedPurchaser = useMemo(
    () => purchasers.find((item) => item.id === selectedPurchaserId),
  ...
  const loadCategories = useCallback(() => api("/purchasers/categories").then(setCategories), []);
  const loadPurchasers = useCallback(() => {
    const params = new URLSearchParams();
    if (search) params.set("search", search);
    if (stage) params.set("stage", stage);
    if (categoryId) params.set("categoryId", categoryId);
    return api(`/purchasers?${params.toString()}`).then(setPurchasers);
  }, [categoryId, search, stage]);

  useEffect(() => {
    if (!user || user.role === "MANAGER") return;
    Promise.all([loadCategories(), loadPurchasers()]).catch((error) => setMessage(error.message));
  }, [user, loadCategories, loadPurchasers]);
  ...
  const createCategory = async (event) => {
    ...
    setBusy(true);
    try {
      const saved = await api("/purchasers/categories", {
        method: "POST",
  ...
  const submitPurchaser = async (event) => {
    event.preventDefault();
    setBusy(true);
    try {
      ...
      await Promise.all([loadCategories(), loadPurchasers()]);
    } catch (error) {
      setMessage(error.message);
    } finally {
      setBusy(false);
  ...
  const openComments = async (purchaserId) => {
    setSelectedPurchaserId(purchaserId);
    try {
      const rows = await api(`/remarks/${purchaserId}?targetType=PURCHASER`);
      setComments(rows);
    } catch (error) {
      setMessage(error.message);
    }
  ...
  const addComment = async (event) => {
    ...
    try {
      await api("/remarks/add", {
        method: "POST",
        body: JSON.stringify({ targetType: "PURCHASER", targetId: selectedPurchaserId, text: commentText })
      });
      setCommentText("");
      setMessage("CEO comment added.");
      await openComments(selectedPurchaserId);
  ...
  if (user.role === "MANAGER") {
    return <Shell user={user}><Notice message="Purchasers are managed by executives and reviewed by the CEO." error /></Shell>;
  }
  ...
  return <Shell user={user}>
    <Reveal variant="clip"><header className="page-hero mb-7">
      ...
      <h1 className="mt-2 text-3xl font-black text-violet-950 md:text-4xl">{isCeo ? "Purchaser review" : "Purchaser pipeline"}</h1>
      <p className="mt-2 text-slate-500">{isCeo ? "Review executive purchaser entries by category and add leadership comments." : "Create categories first, then add purchaser entries under the right category."}</p>
    </header></Reveal>
    ...
    <div className={`grid gap-5 ${isExecutive ? "xl:grid-cols-[minmax(0,1fr)_390px]" : "xl:grid-cols-[minmax(0,1fr)_390px]"}`}>
      <Reveal><Card title={isCeo ? "Category-wise purchasers" : "Purchaser list"}>
        <form onSubmit={(event) => { event.preventDefault(); loadPurchasers().catch((error) => setMessage(error.message)); }} className="mb-4 grid gap-2 md:grid-cols-[1fr_180px_190px_auto]">
        ...
        <div className="space-y-5">
          {groupedPurchasers.map((group) => <section key={group.id} className="rounded-2xl border border-violet-100 bg-white p-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div><h2 className="font-black text-violet-950">{group.name}</h2><p className="text-xs text-slate-400">{group.items.length} purchasers - {group.totalValue.toLocaleString()} expected value</p></div>
            </div>
            <div className="overflow-x-auto"><table className="w-full min-w-[64rem]"><thead><tr><th>Company</th><th>Executive</th><th>Contact</th><th>Interest</th><th>Stage</th><th>Value</th><th>Next follow-up</th><th></th></tr></thead><tbody>{group.items.map((item) => <tr key={item.id}><td className="font-bold text-violet-950">{item.company_name}<div className="text-xs font-normal text-slate-400">{item.city || "-"}</div></td><td>{item.created_by_name}</td><td>{item.contact_person}<div className="text-xs text-slate-400">{item.phone || item.email || "-"}</div></td><td>{item.product_interest || "-"}</td><td><Status value={item.purchase_stage} /></td><td>{item.expected_value ? Number(item.expected_value).toLocaleString() : "-"}</td><td>{item.next_followup_date ? new Date(item.next_followup_date).toLocaleDateString() : "-"}</td><td>{isExecutive ? <button type="button" onClick={() => { setEditingId(item.id); setForm(toForm(item)); }} className="inline-flex items-center gap-1 font-bold text-violet-700"><Edit3 size={14} />Edit</button> : <button type="button" onClick={() => openComments(item.id)} className="inline-flex items-center gap-1 font-bold text-violet-700"><MessageSquareText size={14} />Comment {item.ceo_comment_count ? `(${item.ceo_comment_count})` : ""}</button>}</td></tr>)}</tbody></table></div>
          </section>)}
          {!groupedPurchasers.length && <div className="rounded-2xl bg-violet-50 p-6 text-sm text-slate-500">No purchasers match the current filters.</div>}
        </div>
      </Card></Reveal>
      ...
      {isExecutive && <Reveal delay={100} className="space-y-5">
        <Card title="Create category" action={<Tags size={18} className="text-violet-500" />}>
          <form onSubmit={createCategory} className="space-y-3">
            <div><label>Category</label><input required value={categoryForm.name} onChange={(event) => setCategoryForm({ ...categoryForm, name: event.target.value })} placeholder="Juice companies" /></div>
        ...
        <Card title={editingId ? "Edit purchaser" : "Add purchaser"} action={<Plus size={18} className="text-violet-500" />}>
          <form onSubmit={submitPurchaser} className="space-y-3">
      ...
      {isCeo && <Reveal delay={100}><Card title="CEO comment" action={<MessageSquareText size={18} className="text-violet-500" />}>
        {selectedPurchaser ? <div className="space-y-4">
          <div className="rounded-2xl bg-violet-50 p-4"><div className="font-black text-violet-950">{selectedPurchaser.company_name}</div><div className="text-sm text-slate-500">{selectedPurchaser.category_name || "Uncategorized"} - {selectedPurchaser.created_by_name}</div></div>
        ...
        </div> : <div className="rounded-2xl bg-violet-50 p-6 text-sm text-slate-500">Choose a purchaser from the list to add a CEO comment.</div>}
      </Card></Reveal>}
    </div>
  </Shell>;
}
